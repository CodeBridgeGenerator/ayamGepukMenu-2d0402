...
const InventoryCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            stockNo: _entity?.stockNo,itemName: _entity?.itemName,currentStock: _entity?.currentStock,unit: _entity?.unit,reorderLevel: _entity?.reorderLevel,supplier: _entity?.supplier,
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber tooltip="Use only numbers" id="currentStock" className="w-full mb-3 p-inputtext-sm" value={_entity?.currentStock} onChange={(e) => setValByKey("currentStock", e.value)}  />
            </span>
            <small className="p-error">
