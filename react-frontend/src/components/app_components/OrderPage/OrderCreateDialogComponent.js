...
const OrderCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            receiptNumber: _entity?.receiptNumber,dateTime: _entity?.dateTime,itemsOrdered: _entity?.itemsOrdered,total: _entity?.total,paymentMethod: _entity?.paymentMethod,type: _entity?.type,
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber tooltip="Use only numbers" id="dateTime" className="w-full mb-3 p-inputtext-sm" value={_entity?.dateTime} onChange={(e) => setValByKey("dateTime", e.value)}  />
            </span>
            <small className="p-error">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber tooltip="Use only numbers" id="total" className="w-full mb-3 p-inputtext-sm" value={_entity?.total} onChange={(e) => setValByKey("total", e.value)}  />
            </span>
            <small className="p-error">
