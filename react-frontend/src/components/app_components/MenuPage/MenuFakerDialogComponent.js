...
import menuFakerFactory from "./menuFakerFactory";
...
const MenuFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = menuFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("menu").create(elem));
...
export default connect(mapState, mapDispatch)(MenuFakerDialogComponent);
